...
-- Add '我已处理' menu under '审批中心' (Idempotent by ID)
...
VALUES (
    '9f1b2c3d4e5f60718293a4b5c6d7e8f7', -- id
    '9f1b2c3d4e5f60718293a4b5c6d7e8f1', -- parent_id
    '我已处理', -- name
    '/bpm/done', -- url
    'bpm/done/index', -- component
    'bpm-done', -- component_name
    1, -- menu_type
    NULL, -- perms
    '1', -- perms_type
    5, -- sort_no
...
-- Bind '我已处理' menu to admin role (Idempotent)
...
SELECT
    ...
    '9f1b2c3d4e5f60718293a4b5c6d7e8f7', -- permission_id
    NULL, -- data_rule_ids
    NOW(), -- operate_date
...
WHERE NOT EXISTS (
    ...
    WHERE role_id = (SELECT id FROM sys_role WHERE role_code = 'admin') 
      AND permission_id = '9f1b2c3d4e5f60718293a4b5c6d7e8f7'
);
