...
select replace(uuid(),'-',''), 'Manager', 'MANAGER', 'flowable-mvp4', now(), '${ADMIN_USER}'
from dual where not exists (select 1 from sys_role where role_code='MANAGER');" >/dev/null
mysql_exec "insert into sys_role (id, role_name, role_code, description, create_time, create_by)
select replace(uuid(),'-',''), 'Finance', 'FINANCE', 'flowable-mvp4', now(), '${ADMIN_USER}'
...
run_branch() {
  ...
  if [ "$USED_PROCESS_KEY" != "$PROCESS_KEY" ]; then
    echo "Unexpected process key: $START_RES" >&2
    exit 1
  fi
  ...
  if [ "$NEXT_GROUP" != "$expect_group" ]; then
    echo "Unexpected candidate group: $STATUS_NEXT" >&2
    exit 1
  fi

  MANAGER_VISIBLE=$(visible_in_tasks "$MANAGER_TOKEN" "$NEXT_TASK_ID")
  FINANCE_VISIBLE=$(visible_in_tasks "$FINANCE_TOKEN" "$NEXT_TASK_ID")
  if [ "$expect_group" = "MANAGER" ]; then
    if [ "$MANAGER_VISIBLE" != "true" ] || [ "$FINANCE_VISIBLE" != "false" ]; then
      echo "Visibility check failed (MANAGER branch): manager=${MANAGER_VISIBLE} finance=${FINANCE_VISIBLE}" >&2
      exit 1
    fi
  else
    if [ "$MANAGER_VISIBLE" != "false" ] || [ "$FINANCE_VISIBLE" != "true" ]; then
      echo "Visibility check failed (FINANCE branch): manager=${MANAGER_VISIBLE} finance=${FINANCE_VISIBLE}" >&2
      exit 1
    fi
  fi
...
run_branch "A_amount_20001" 20001 "$MANAGER_TOKEN" "$FINANCE_TOKEN" "MANAGER"
run_branch "B_amount_99" 99 "$FINANCE_TOKEN" "$MANAGER_TOKEN" "FINANCE"
