...
const FORM_KEY = process.env.FORM_PROCESS_KEY || 'test_menu_form_01';
...
(async () => {
  ...
  try {
    if (OA_STORAGE_STATE && fs.existsSync(OA_STORAGE_STATE)) {
      ...
      await gotoWithRetries(`${BASE_URL}/form/designer?formKey=${FORM_KEY}`);
    } else {
    ...
    const processTab = page.locator('.ant-tabs-tab').filter({ hasText: '流程设计' }).first();
    if (!(await processTab.count())) {
      const skipShot = path.join(ARTIFACTS_DIR, 'no-process-tab.png');
      try {
    ...
    failureStage = 'wait-canvas';
    const canvas = page.locator('[data-testid="form-bpmn-canvas"]').first();
    ...
    const publishBtn = page.locator('button:has-text("发布部署")').first();
